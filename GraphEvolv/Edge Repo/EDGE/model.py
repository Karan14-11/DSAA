...
from models.sparse_dycond import SparseDyCondGNN
...
def add_model_args(parser):
    ...
    parser.add_argument('--arch', type=str, help='GAT | TGNN | SparseDyCond') # <--- Added Option
    ...
    parser.add_argument('--max_nodes', type=int, default=20000, help='Max node count for embeddings')
...
def get_model(args, initial_graph_sampler):
    ...
    if args.arch == 'SparseDyCond':
        print(f"Initializing SparseDyCond with Max Nodes: {args.max_nodes}")
        model = SparseDyCondGNN(
            num_nodes=args.max_nodes,
            emb_dim=args.emb_dim,
            # num_layers=len(args.num_heads), # Use length of heads list as layer count
            # num_classes=2
        )
        return model
